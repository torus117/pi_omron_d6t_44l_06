...
D6T::D6T(std::string i2c_bus, uint8_t addr, uint8_t inType)
{
    //if(addr != 0x14)
...
uint8_t D6T::transfer(uint8_t writeBytes, uint8_t* pWrite, uint8_t readBytes, uint8_t* pRead)
{
    int result;
...
uint8_t D6T::checkPEC(uint8_t* buffer, uint8_t length)
{
    uint8_t crc;
    uint8_t i;
